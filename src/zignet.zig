...
pub const Socket = struct {
    ...
    pub const Writer = switch (builtin.os.tag) {
        .windows => struct {
            ...
            fn addWsaBuf(v: []std.windows.ws2_32.WSABUF, i: *u32, bytes: []const u8) void {
            ...
            fn drain(io_w: *std.Io.Writer, data: []const []const u8, splat: usize) std.Io.Writer.Error!usize {
                ...
                var iovecs: [max_buffers_len]std.windows.ws2_32.WSABUF = undefined;
                var len: u32 = 0;
                ...
                const n = sendBufs(w.stream.handle, iovecs[0..len]) catch |err| n: {
            ...
            fn handleSendError(winsock_error: std.windows.ws2_32.WinsockError) Writer.Error!void {
                switch (winsock_error) {
            ...
            fn sendBufs(fd: std.posix.socket_t, bufs: []std.windows.ws2_32.WSABUF) Writer.Error!u32 {
                var n: u32 = undefined;
                var overlapped: std.windows.OVERLAPPED = std.mem.zeroes(std.windows.OVERLAPPED);
                if (std.windows.ws2_32.WSASend(
                    ...
                    &overlapped,
                    null,
                ) == std.windows.ws2_32.SOCKET_ERROR) switch (std.windows.ws2_32.WSAGetLastError()) {
                    .WSA_IO_PENDING => {
                        var result_flags: u32 = undefined;
                        if (std.windows.ws2_32.WSAGetOverlappedResult(
                            ...
                            std.windows.TRUE,
                            &result_flags,
                        ) == std.windows.FALSE) try handleSendError(std.windows.ws2_32.WSAGetLastError());
                    },
            ...
            fn drain(io_w: *std.Io.Writer, data: []const []const u8, splat: usize) std.Io.Writer.Error!usize {
                ...
                return io_w.consume(std.posix.sendmsg(w.fd, &msg, flags) catch |err| {
